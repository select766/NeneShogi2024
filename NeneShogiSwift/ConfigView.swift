...
private let userDefaultsCSAConfigSaveKey = "csaConfigSave"
private let userDefaultsUSIServerIpAddressKey = "usiServerIpAddress"
...
enum EngineStartInfo {
    case USI(usiConfig: USIConfig)
    case CSA(csaConfig: CSAConfig)
}
...
struct ConfigView: View {
    ...
    var onStart: (EngineStartInfo) -> Void
    
    init(onStart: @escaping (EngineStartInfo) -> Void) {
    ...
    func start(serverType: String) {
        ...
        if serverType == "USI" {
            let usiConfig = USIConfig(usiServerIpAddress: usiServerIpAddress, usiServerPort: 8090, ponder: true)
            onStart(.USI(usiConfig: usiConfig))
        } else if serverType == "CSA" {
            let csaConfig = formToCSAConfig()
            onStart(.CSA(csaConfig: csaConfig))
        }
    }
